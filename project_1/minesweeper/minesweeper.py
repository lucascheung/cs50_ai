...
class MinesweeperAI():
    ...
    def add_knowledge(self, cell, count):
        ...
        # remove neighbors that we know the states
        mine_counts = count
        ...
        # add a new sentence
        new_sentence = Sentence(neighbors, mine_counts)
    ...
    def find_all_neighbors(self, cell):
        ...
        for r in range(-1,2):
            for c in range(-1,2):
                if 0 <= i + r < self.height and 0 <= j + c < self.width:
                    if not (r == 0 and c == 0):
                        neighbors.add((i+r, j+c))
        return neighbors
